...
# ----------------------------------------------------------
# ۲. موتور صوتی چندزبانه جارویس / اپتیموس پرایم (با مدیریت خطای ایمن)
# ----------------------------------------------------------
...
def create_voice_file(text, filename, mode="jarvis"):
    ...
    try:
        ...
        logging.error(f"خطای gTTS در ساخت ویس: {e}")
        try:
            tts = gTTS(text=text, lang='en', slow=False)
            tts.save(filename)
        except Exception as inner_e:
            logging.error(f"خطای دوم ساخت ویس: {inner_e}")
            
    return filename

# ----------------------------------------------------------
# ۳. هسته هوشمند ۳ لایه جمنای ۵۴۰۰ (بهینه‌سازی شده و ضد محدودیت 429)
# ----------------------------------------------------------
...
# ----------------------------------------------------------
# ۹. اجرای هم‌زمان وب‌سرور و ربات تلگرام (با تنظیمات تایم‌اوت مقاوم)
# ----------------------------------------------------------
if __name__ == "__main__":
    # استارت سرور Flask در یک ترد جداگانه
    ...
    # اجرای پولینگ مقاوم با پتانسیل تایم‌اوت بهینه‌شده
    while True:
        try:
            asyncio.run(bot.infinity_polling(timeout=90, request_timeout=60, skip_pending=True))
        except Exception as err:
            logging.error(f"خطای ارتباطی پش‌ورود (تلاش مجدد خودکار): {err}")
            time.sleep(5)
