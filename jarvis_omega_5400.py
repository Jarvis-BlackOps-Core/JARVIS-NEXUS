...
# ==========================================================
# J.A.R.V.I.S. // PROTOCOL 920 - OMEGA GOD-MODE 5400 (FIXED & ULTRA-STABLE)
...
def run_web_server():
    ...
    # استفاده از wsgiref برای جلوگیری از بلاک شدن و تداخل روی Render
...
# ----------------------------------------------------------
# ۲. موتور صوتی چندزبانه جارویس / اپتیموس پرایم (کاملاً اصلاح‌شده)
# ----------------------------------------------------------
...
def clean_text_for_speech(text):
    # پاک‌سازی کدهای ایموجی و علامت‌های مارک‌داون برای خوانش شفاف gTTS
...
def create_voice_file(text, filename, mode="jarvis"):
    try:
        ...
        # خلاصه کردن متن برای ویس جهت سرعت بالا و عدم قطع شدن
        voice_text = clean_t[:250] if len(clean_t) > 250 else clean_t
        if not voice_text:
            voice_text = "درود ارباب رضا"

        if mode == "prime":
            prime_prefix = "گزارش اپتیموس پرایم: " if lang == 'fa' else "Autobot Prime Report: "
...
# ----------------------------------------------------------
# ۳. هسته هوشمند جمنای (مدل‌های هوشمند و فعال)
# ----------------------------------------------------------
async def ask_gemini_5400(prompt, preferred_model="flash"):
    ...
    for model_name in models_chain:
        try:
            ...
            res = await asyncio.wait_for(loop.run_in_executor(executor, call_api), timeout=15.0)
            if res and res.text:
...
async def generate_ai_image(prompt_text):
    ...
    try:
        ...
        result = await asyncio.wait_for(loop.run_in_executor(executor, call_imagen), timeout=30.0)
...
# ----------------------------------------------------------
# ۶. ماژول حذف هوشمند واترمارک/ادساین ویدیو با FFmpeg
# ----------------------------------------------------------
...
# ----------------------------------------------------------
# ۱۲. اجرای پایدار (حذف وب‌هوک‌های قبلی جهت رفع Conflict)
# ----------------------------------------------------------
async def main():
    # اجرای وب‌سرور در یک ترد مجزا با wsgiref
    server_thread = threading.Thread(target=run_web_server, daemon=True)
    server_thread.start()

    # پاک‌سازی وب‌هوک و جلسات قبلی تلگرام برای جلوگیری از ارور 409 Conflict
    try:
