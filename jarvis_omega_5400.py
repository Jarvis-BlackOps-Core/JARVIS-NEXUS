...
# ==========================================================
# J.A.R.V.I.S. // PROTOCOL 920 - OMEGA GOD-MODE 5400 (ULTIMATE)
...
# ----------------------------------------------------------
# ۳. هسته هوشمند جمنای (پاسخ‌دهی متنی)
# ----------------------------------------------------------
async def ask_gemini_5400(prompt, preferred_model="flash"):
    ...
    for model_name in models_chain:
        try:
            ...
            res = await asyncio.wait_for(loop.run_in_executor(executor, call_api), timeout=8.0)
            if res and res.text:
...
def remove_video_watermark(input_path, output_path, position="bottom"):
    """
    حذف واترمارک و ادساین با اعمال ماسک تاری (Blur) روی ویدیو
    """
    try:
        # تنظیم مختصات تاری بر اساس محل قرارگیری ادساین (پایین یا بالا)
        if position == "bottom":
...
async def quantum_main_engine(message):
    ...
    # ۴. گفتگوی عمومی جارویس + پاسخ متنی و ویس
    else:
        try:
