...
# شناسه و آیدی عددی فرماندهان ارشد (Root Access)
MASTER_USERNAMES = ["reza_mohafez1", "jarvis_core_x"]
MASTER_NUMERIC_IDS = [92814921, 8940874598]

# زیرسیستم‌های متصل به هسته مرکزی جارویس
SUBSYSTEMS = {
...
def is_master_user(user):
    """احراز هویت فرمانده با آیدی عددی و یوزرنیم"""
...
# ----------------------------------------------------------
# ۳. هسته هوشمند جمنای با دستورالعمل جامع سیستم
# ----------------------------------------------------------
...
# ----------------------------------------------------------
# ۵. دانلودر توربو با موتور دوگانه (yt-dlp + Cobalt API Engine)
# ----------------------------------------------------------
def download_media_ultra(url):
    os.makedirs('downloads', exist_ok=True)
    
    # ۱. تلاش اول: موتور اصلی yt-dlp با کلاینت‌های متغیر
    ydl_opts = {
    ...
    try:
        ...
        logging.warning(f"yt-dlp encountered restriction, switching to Cobalt API Bypass: {e}")
        
        # ۲. سوییچ هوشمند به موتور دوم جهت دور زدن کامل محدودیت آی‌پی سرور رندر
        try:
            headers = {
            ...
            for endpoint in api_endpoints:
                try:
                    ...
                    if res.status_code == 200:
                        ...
                        if dl_link:
                            file_path = f"downloads/yt_media_{int(time.time())}.mp4"
        ...
        raise Exception("محدودیت یوتیوب؛ تلاش با موتور ثانویه نیز ناموفق بود.")
...
# ----------------------------------------------------------
# ۷. سیستم پادشاهی کدهای هدیه و ردیم (JARVIS GOD-MODE REDEEM ENGINE)
# ----------------------------------------------------------
...
# ----------------------------------------------------------
# ۸. دستورات شبکه ۹۲۰ و کانفیگ‌های کوانتومی
# ----------------------------------------------------------
...
# ----------------------------------------------------------
# ۱۲. هسته اصلی پردازش هوشمند و ارجاع به زیرسیستم‌ها
# ----------------------------------------------------------
...
async def quantum_main_engine(message):
    ...
    if any(text.startswith(p) for p in ["عکس بساز:", "تصویر بساز:", "تصویر:", "عکس:"]):
        ...
        await bot.reply_to(message, "🎨 [ماژول طراحی]: در حال پردازش و خلق تصویر با استاندارد ۵۴۰۰...")
    ...
    # ۲. حالت گفتگوی حماسی اپتیموس پرایم
    ...
    # ۳. ارجاع به زیرسیستم دانلودر YouTube Dark
    ...
    # ۴. گفتگوی جامع و تحلیل هوشمند جارویس (پاسخ‌های کامل)
    else:
        try:
